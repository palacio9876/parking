...
const updateCompanyDto = z.object({
  ...
  tax_id: z.string().trim().min(1, 'tax_id is required').optional(),
...
const updateSettingsDto = z.object({
  ...
  opening_time: z.string().trim().regex(/^\d{2}:\d{2}(?::\d{2})?$/).optional(),
  closing_time: z.string().trim().regex(/^\d{2}:\d{2}(?::\d{2})?$/).optional(),
